...
class DataSet():
    def __init__(self, model=None, target='train', batch_size=2, width=None, height=None, dim=3, filled=False):
        ...
        for x in uu_all_images:
            uu.append(os.path.basename(x))  # 自动识别系统路径分隔符
    ...
    def batch_generator(self, images, batch_size, target='train'):
        ...
        while True:
            ...
            for i in ix:
                ...
                transformed = transform(image=image0, mask=image1,
                                        mask0=mask)  # treating ADI as a mask here as we want to avoid color transformations to it.
                transformed2 = transform2(image=image0, mask=image1,
                                          mask0=mask)  # treating ADI as a mask here as we want to avoid color transformations to it.
