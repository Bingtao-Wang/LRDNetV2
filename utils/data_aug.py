...
class DataSet_aug():
    def __init__(self, model=None, target='train', batch_size=2, width=None, height=None, dim=3):
        ...
        if os.path.exists(self.image_data_folder):
            all_images = os.listdir(self.image_data_folder)
            all_images.sort()
        ...
        if os.path.exists(self.image_data_folder_val):
            all_images_val = os.listdir(self.image_data_folder_val)
            all_images_val.sort()
            all_images_val = np.array(all_images_val)
        else:
            all_images_val = np.array([])
    ...
    def batch_generator(self, images, batch_size, target):
        while True:
            if images.shape[0] == 0:
                print("Warning: No images found for augmentation generator.")
                break
